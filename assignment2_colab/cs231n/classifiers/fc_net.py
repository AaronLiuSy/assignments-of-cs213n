...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3 * 32 * 32, hidden_dim=100, num_classes=10, weight_scale=1e-3, reg=0.0):
        ...
        self.params['W1'] = np.random.normal(0, weight_scale, size=(input_dim,hidden_dim)) # mean=0, std=weight_scale
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers - 1):
            if i == 0:
                ...
                self.params['W%s' %(i+1)] = np.random.normal(0, weight_scale, size=(hidden_dims[i-1], hidden_dims[i]))
                self.params['b%s' %(i+1)] = np.zeros(shape=(hidden_dims[i]))
                if self.normalization is not None:
                    self.params['gamma%s' % (i+1)] = np.ones(shape=(hidden_dims[i]))
                    self.params['beta%s' % (i+1)] = np.zeros(shape=(hidden_dims[i]))
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            cache = list()
            # scores, fc_cache = affine_relu_forward(scores, self.params['W%s' %(i+1)], self.params['b%s' %(i+1)])
            ...
            cache.append(fc_cache)
            if self.normalization == 'batchnorm':
                scores, bn_cache = batchnorm_forward(scores, self.params['gamma%s'%(i+1)], self.params['beta%s'%(i+1)], self.bn_params[i])
                cache.append(bn_cache)
            elif self.normalization == 'layernorm':
                scores, ln_cache = layernorm_forward(scores, self.params['gamma%s'%(i+1)], self.params['beta%s'%(i+1)], self.bn_params[i])
            ...
            caches.append(cache)
        scores, fc_cache = affine_forward(scores, self.params['W%s' %self.num_layers], self.params['b%s' %self.num_layers])
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        loss, dx = softmax_loss(scores, y) # without regularization
        ...
        # grads
        # computation graph: affine --> bn(Op) --> relu --> dropout(Op)
